import re,requests
def load_log(path):
    pattern = r'^(\S+) .*:(\d{2}):\d{2}:\d{2} .* "\S+ (\S+) \S+" (\d+)'
    records=[]
...
def build_text(stats):
    ...
    text += f"攻击高峰: {stats['top_hour']}\n"
    return text
def send_feishu(text):
    url="https://open.feishu.cn/open-apis/bot/v2/hook/e89db514-10ae-43c1-9d32-b28ffec2b218"
    msg = {
      "msg_type": "text", 
      "content": {"text": text}
    }
